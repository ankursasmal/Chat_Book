...
import MessangerNav from './MessangerNav';
import { PiDotsNineBold } from "react-icons/pi";
import NavSqureMenu from './NavSqureMenu';
import CreateContentsNav from './CreateContentsNav';

 
function Nav3() {
  let [clickProfile,setclickProfile]=useState(false);
  let displayProfile=useRef('');
  let [clickNotification,setclickNotification]=useState(false);
  let displayNotification=useRef('');
  let [clickChat,setclickChat]=useState(false);
  let displayChat=useRef('');
  ...
  return (
    <div>
       <div  ref={displayProfile} style={{display:'none'}} >< Modal_profile /></div>
       <div  ref={displayNotification} style={{display:'none'}} >< NotificationNav /></div>
       <div  ref={displayChat} style={{display:'none'}} >< MessangerNav /></div>
       <div  ref={displayclickSqureDot} style={{display:'none'}} >< NavSqureMenu /></div>
       <div  ref={create} style={{display:'none'}} >< CreateContentsNav /></div>

      <div className=' item-center ml-4 mr-2'>
  <div className='flex items-center  justify-around py-2'>
  <PiDotsNineBold  className='text-7xl px-4  lg:block  hidden ' onClick={(e)=>{
      e.preventDefault();
...
if(clickCreate===true){
  ...
  <FaFacebookMessenger  className='text-4xl '   onClick={(e)=>{
      e.preventDefault();
if(clickChat===true){
  setclickChat(false);
  displayChat.current.style.display='block';
}  
else{
  setclickChat(true);
  displayChat.current.style.display='none';

}    
     }} />
  <FaRegBell className='text-6xl p-4' onClick={(e)=>{
      e.preventDefault();
if(clickNotification===true){
...
if(clickProfile===true){
  ...
  displayProfile.current.style.display='block';
}  
else{
  setclickProfile(true);
  displayProfile.current.style.display='none';
