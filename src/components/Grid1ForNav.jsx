import React, { useState } from 'react'
import Title     from './Title'
import { Link } from "react-router-dom";


import { AiOutlineVideoCamera } from "react-icons/ai";
import { RiMemoriesFill } from "react-icons/ri";
import { CiBookmark } from "react-icons/ci";
import { MdGroup } from "react-icons/md";
import { FaShop } from "react-icons/fa6";
import { BiCalendarStar } from "react-icons/bi";
import { BiBookAdd } from "react-icons/bi";
import { IoChevronDownSharp } from "react-icons/io5";
import { MdOutlineKeyboardArrowUp } from "react-icons/md";
import { FaFacebookMessenger } from "react-icons/fa6";
import { PiMessengerLogoThin } from "react-icons/pi";
import { IoFlagSharp } from "react-icons/io5";
import { FaGamepad } from "react-icons/fa";
import { CiImageOn } from "react-icons/ci";
import { MdOutlineCopyright } from "react-icons/md";
import { TbAlertTriangle } from "react-icons/tb";

function Grid1ForNav() {
      let [seen,setseen]=useState(true);

  return (
    <div   className='bg-white relative z-30 '   >
     <div className='pl-1 md:pl-3  md:mr-6  mr-[3px]   overflow-y-auto h-[75vh] w-[36%] md:w-[26%] flex flex-col bg-white fixed top-[74px] left-0' style={{backgroundColor:'white'}} >
      <div className='flex flex-col  z-30  '> 
        <div className='flex items-center mb-3 mt-4 z-30  '>
     <img className='border rounded-full w-[40px] h-[40px] md:mr-4 mr-0 hidden md:block'  src="data:image/jpeg;base64," alt=""   />
              <a className="md:font-normal  font-bold ">  <Title      name={'User'}/></a>

        </div>
        <Link to='/friends'>  <div className='flex items-center '>
            <img className='border rounded-full w-[40px] h-[40px] md:mr-4 mr-0 hidden md:block'  src="  https://www.shutterstock.com/image-vector/facebook-friends-user-interface-icon-260nw-465585674.jpg" alt=""   />
              <a className="md:font-normal  font-bold ">  <Title   name={'Friends'}/></a>

        </div></Link>
        <Link to='/vedios'>  <div className='flex items-center my-3'>
        <AiOutlineVideoCamera className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Vedios'}/></a>

        </div></Link>

      <Link to='/memories'>   <div className='flex items-center my-3'>
        <RiMemoriesFill className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Memorie'}/></a>

        </div></Link>
      
        <Link to='/save'>  <div className='flex items-center my-3'>
        <CiBookmark className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Saved'}/></a>

        </div>
        </Link>
         
        <Link to='/groups'>   <div className='flex items-center my-3'>
        <MdGroup className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Gropus'}/></a>

        </div>
        </Link>
        <Link to='/market-place'><div className='flex items-center my-3'>
        <FaShop className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Market Placess'}/></a>

        </div>
         </Link>
        <Link to='/feeds'>  <div className='flex items-center my-3'>
        <BiBookAdd className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Feeds'}/></a>

        </div></Link>
        <Link to='/test'>   <div className='flex items-center my-3'>
        <BiCalendarStar className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Events'}/></a>

        </div></Link>
       <div className='flex items-center my-3'>
       <img className='border rounded-full w-[40px] h-[40px] mr-3 hidden md:block'  src=" https://img.myloview.com/posters/line-graph-icon-isolated-on-white-background-statistics-symbol-modern-simple-vector-icon-for-website-design-mobile-app-ui-vector-illustration-700-255818520.jpg" alt=""   />
               <a className="md:font-normal  font-bold ">  <Title      name={'AddMannager'}/></a>

        </div>
        <div className='flex items-center my-3'>
        <img className='border rounded-full w-[40px] h-[40px] mr-3 hidden md:block'  src="         https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjygy9jFmEUl0md_mUUNkajw32l_WpjZrnKg&s
" alt=""   />
               <a className="md:font-normal  font-bold ">  <Title      name={'Fundraisers'}/></a>

        </div>
        
        </div>
         
     {seen?   <div className='flex items-center my-2' onClick={()=>{setseen(false)}}>
        <IoChevronDownSharp className='md:mx-4   mx-[0px]'/>
                 <a className="md:font-normal  font-bold ">  <Title      name={'See'}  /></a>

        </div>
        :
        <div className='flex flex-col '> 
        <div className='flex items-center my-3'>
        <img className='border rounded-full w-[40px] h-[40px] mr-3 hidden md:block'  src="https://t3.ftcdn.net/jpg/02/15/57/22/360_F_215572243_4HMbV2462b7m3nlRHwHxatX3fRLCfLTa.jpg" alt=""   />
               <a className="md:font-normal  font-bold ">  <Title      name={'Blood Donation'}/></a>

        </div>
        <div className='flex items-center my-3'>
        <img className='border rounded-full w-[40px] h-[40px] mr-3 hidden md:block'  src="        https://static.vecteezy.com/system/resources/thumbnails/001/312/526/small_2x/planet-earth-with-plant-growing-free-vector.jpg
 " alt=""   />
               <a className="md:font-normal  font-bold ">  <Title      name={'Climet Science Center'}/></a>

        </div>
        <div className='flex items-center my-3'>
        <FaFacebookMessenger className='md:mx-4 md:block hidden mx-[0px]'/>

               <a className="md:font-normal  font-bold ">  <Title      name={'Messenger'}/></a>

        </div>
        <div className='flex items-center my-3'>
        < PiMessengerLogoThin className='md:mx-4 md:block hidden mx-[0px]'/>
                <a className="md:font-normal  font-bold ">  <Title      name={' Messanger Kids'}/></a>

        </div>
        <div className='flex items-center my-3'>
        < IoFlagSharp className='md:mx-4 md:block hidden mx-[0px]'/>
                <a className="md:font-normal  font-bold ">  <Title      name={'Pages'}/></a>

        </div>
        <div className='flex items-center my-3'>
        < FaGamepad className='md:mx-4 md:block hidden mx-[0px]'/>
                <a className="md:font-normal  font-bold ">  <Title      name={'Play Games'}/></a>

        </div>
        
        <div className='flex items-center my-3'>
        < CiImageOn className='md:mx-4 md:block hidden mx-[0px]'/>
                <a className="md:font-normal  font-bold ">  <Title      name={'Recents Ad And Activity'}/></a>

        </div>
         <div className='flex items-center my-2' onClick={()=>{setseen(true)}}>
         <MdOutlineKeyboardArrowUp className='md:mx-4   mx-[0px]'/>
                  <a className="md:font-normal  font-bold ">  <Title      name={'See'}  /></a>
 
         </div>
        </div>
         
        } 
         
</div>
</div>

  )
}

export default Grid1ForNav
