import React, { useState,useRef } from 'react'
import Title     from './Title'
import { Link } from "react-router-dom";


import { AiOutlineVideoCamera } from "react-icons/ai";
import { RiMemoriesFill } from "react-icons/ri";
import { CiBookmark } from "react-icons/ci";
import { MdGroup } from "react-icons/md";
import { FaShop } from "react-icons/fa6";
import { BiCalendarStar } from "react-icons/bi";
import { BiBookAdd } from "react-icons/bi";
import { IoChevronDownSharp } from "react-icons/io5";
import { MdOutlineKeyboardArrowUp } from "react-icons/md";
import { FaFacebookMessenger } from "react-icons/fa6";
import { PiMessengerLogoThin } from "react-icons/pi";
import { IoFlagSharp } from "react-icons/io5";
import { FaGamepad } from "react-icons/fa";
import { CiImageOn } from "react-icons/ci";
import { MdOutlineCopyright } from "react-icons/md";
import { TbAlertTriangle } from "react-icons/tb";

function Grid1ForNav() {
      let [seen,setseen]=useState(true);
 

  return (
    <div   className='bg-white relative z-30  h-[100vh] '   >
     <div className='pl-1 md:pl-3  md:mr-6  mr-[3px] rounded-lg  overflow-y-auto h-[96vh]  w-[200px] md:w-[26%] flex flex-col bg-white fixed top-[74px] left-0' style={{backgroundColor:'white'}} >
      <div className='flex flex-col  z-30  '> 
        <div className='flex items-center mb-3 mt-4 z-30  '>
     <img className='border rounded-full w-[40px] h-[40px] md:mr-4 mr-1  '  src="data:image/jpeg;base64," alt=""   />
     <Link to='/'>    <Title    name={'User'} /></Link> 

        </div>
        <Link to='/friends'>  <div className='flex items-center '>
            <img className='border rounded-full w-[40px] h-[40px] md:mr-4 mr-1  '  src="  https://www.shutterstock.com/image-vector/facebook-friends-user-interface-icon-260nw-465585674.jpg" alt=""   />
                <Title   name={'Friends'}  /> 

        </div></Link>
        <Link to='/vedios'>  <div className='flex items-center my-3'>
        <AiOutlineVideoCamera className='md:mx-4  mx-2 '/>

                  <Title      name={'Vedios'}/> 

        </div></Link>

      <Link to='/memories'>   <div className='flex items-center my-3'>
        <RiMemoriesFill className='md:mx-4  mx-2 '/>

                  <Title      name={'Memorie'}/> 

        </div></Link>
      
        <Link to='/save'>  <div className='flex items-center my-3'>
        <CiBookmark className='md:mx-4  mx-2 '/>

                  <Title      name={'Saved'}/> 

        </div>
        </Link>
         
        <Link to='/groups'>   <div className='flex items-center my-3'>
        <MdGroup className='md:mx-4  mx-2 '/>

                  <Title      name={'Gropus'}/> 

        </div>
        </Link>
        <Link to='/market-place'><div className='flex items-center my-3'>
        <FaShop className='md:mx-4  mx-2 '/>

                  <Title      name={'Market Placess'}/> 

        </div>
         </Link>
        <Link to='/feeds'>  <div className='flex items-center my-3'>
        <BiBookAdd className='md:mx-4  mx-2 '/>

                  <Title      name={'Feeds'}/> 

        </div></Link>
        <Link to='/test'>   <div className='flex items-center my-3'>
        <BiCalendarStar className='md:mx-4  mx-2 '/>

                  <Title      name={'Events'}/> 

        </div></Link>
       <div className='flex items-center my-3'>
       <img className='border rounded-full w-[40px] h-[40px] mr-3  '  src=" https://img.myloview.com/posters/line-graph-icon-isolated-on-white-background-statistics-symbol-modern-simple-vector-icon-for-website-design-mobile-app-ui-vector-illustration-700-255818520.jpg" alt=""   />
                  <Title      name={'AddMannager'}/> 

        </div>
        <div className='flex items-center my-3'>
        <img className='border rounded-full w-[40px] h-[40px] mr-3  '  src="         https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjygy9jFmEUl0md_mUUNkajw32l_WpjZrnKg&s
" alt=""   />
                  <Title      name={'Fundraisers'}/> 

        </div>
        
        </div>
         
     {seen?   <div className='flex items-center my-2' onClick={()=>{setseen(false);
visible_hidden.current.style.display='none'}}>
        <IoChevronDownSharp className='md:mx-4  mx-2 '/>
                    <Title      name={'See'}  /> 

        </div>
        :
        <div className='flex flex-col '> 
        <div className='flex items-center my-3'>
        <img className='border rounded-full w-[40px] h-[40px] mr-3  '  src="https://t3.ftcdn.net/jpg/02/15/57/22/360_F_215572243_4HMbV2462b7m3nlRHwHxatX3fRLCfLTa.jpg" alt=""   />
                  <Title      name={'Blood Donation'}/> 

        </div>
        <div className='flex items-center my-3'>
        <img className='border rounded-full w-[40px] h-[40px] mr-3  '  src="        https://static.vecteezy.com/system/resources/thumbnails/001/312/526/small_2x/planet-earth-with-plant-growing-free-vector.jpg
 " alt=""   />
                  <Title      name={'Climet Science Center'}/> 

        </div>
        <div className='flex items-center my-3'>
        <FaFacebookMessenger className='md:mx-4  mx-2 '/>

                  <Title      name={'Messenger'}/> 

        </div>
        <div className='flex items-center my-3'>
        < PiMessengerLogoThin className='md:mx-4  mx-2 '/>
                   <Title      name={' Messanger Kids'}/> 

        </div>
        <div className='flex items-center my-3'>
        < IoFlagSharp className='md:mx-4  mx-2 '/>
                   <Title      name={'Pages'}/> 

        </div>
        <div className='flex items-center my-3'>
        < FaGamepad className='md:mx-4  mx-2 '/>
                   <Title      name={'Play Games'}/> 

        </div>
        
        <div className='flex items-center my-3'>
        < CiImageOn className='md:mx-4  mx-2 '/>
                   <Title      name={'Recents Ad And Activity'}/> 

        </div>
         <div className='flex items-center my-2' onClick={()=>{setseen(true)}}>
         <MdOutlineKeyboardArrowUp className='md:mx-4  mx-2 '/>
                     <Title      name={'See'}  /> 
 
         </div>
        </div>
         
        } 
         
         
</div>
</div>

  )
}

export default Grid1ForNav
